...
var formatRegExp, debuglog, inspect2, types = () => {}, months, promisify, TextEncoder, TextDecoder2;
var init_util = __esm(() => {
  formatRegExp = /%[sdj%]/g;
...
function getOrCreateDappKeyPair() {
  ...
  if (saved === null) {
    const dappKeyPair = import_tweetnacl.default.box.keyPair();
    localStorage.setItem("dappKeyPair", esm_default2.encode(dappKeyPair.secretKey));
    return dappKeyPair;
  } else {
    return import_tweetnacl.default.box.keyPair.fromSecretKey(esm_default2.decode(saved));
  }
}
...
function parseConnectResponse(dappKeyPair) {
  const { phantom_encryption_public_key, nonce, data } = Object.fromEntries(new URLSearchParams(window.location.search));
  const phantomPublicKey = esm_default2.decode(phantom_encryption_public_key);
  const nonceBytes = esm_default2.decode(nonce);
  const encryptedData = esm_default2.decode(data);
  const decryptedBytes = import_tweetnacl.default.box.open(encryptedData, nonceBytes, phantomPublicKey, dappKeyPair.secretKey);
  const decrypted = new TextDecoder().decode(decryptedBytes);
  return JSON.parse(decrypted);
}
function connectWallet(dappKeyPair, walletElement, imgElement) {
  try {
    const { public_key, session } = parseConnectResponse(dappKeyPair);
...
var dappKeyPair = getOrCreateDappKeyPair();
var link = document.querySelector("#connect");
link.href = createConnectUrl(dappKeyPair);
var walletElement = document.querySelector("#wallet");
